...
def finding_from_events(
    ...
    *,
    ...
    return Finding(
        ...
        created_at=utcnow(),
        attributes={
            "src_ip": trigger.src_ip or "",
            "dst_ip": trigger.dst_ip or "",
            "user": (trigger.user.name if trigger.user and trigger.user.name else ""),
            "host": (trigger.host.name if trigger.host and trigger.host.name else ""),
            "process": (trigger.process.name if trigger.process and trigger.process.name else ""),
        },
    )
